...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($sender_name) || !filter_var($sender_email, FILTER_VALIDATE_EMAIL) || strlen($body) < 20) {
        $msgError = 'Please fill in all fields correctly (message min. 20 characters).';// Validation error
    } else {
...
// display labels and content 
$tagClass = $item['status'] === 'resolved' ? 'tag-resolved' : ($type === 'lost' ? 'tag-lost' : 'tag-found');
$tagText  = $item['status'] === 'resolved' ? 'Resolved' : ucfirst($type);
$dateLabel = $type === 'lost' ? 'Date Lost' : 'Date Found';
$dateVal = $type === 'lost' ? $item['date_lost'] : $item['date_found'];
$isLost = $type === 'lost';
$personLbl = $isLost ? 'Reported by' : 'Found by';
...
<?php else: ?>
            ...
            <div class="detail-text">
                ...
                <div class="detail-info-row"><!--details-->
...
<?php if (!empty($item['reward'])): ?>  <!-- Reward only for lost items-->
    ...
    <div class="right-column">
        ...
        <?php else: ?>
            <div class="side-card" style="border:2px solid #3b82f6">
                <h3><?php echo $isLost ? 'Found This Item?' : 'Is This Yours?'; ?></h3><!--if lost show first, if found show second-->
...
<?php if ($isLost && !empty($item['reward'])): ?><!-- sow reward only for lost-->
